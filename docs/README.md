...
| [`clauditor-sentry-flow.excalidraw`](./clauditor-sentry-flow.excalidraw) | Workflow C — Sentry crash-to-PR. Four sections: **convergence** (many duplicate webhooks → one deduped job), the **5-guard filter pipeline** at the route layer, the **handler gates** (source-map check + branch-exists decision) leading into the Claude run, and the **three-outcome fan-out** with an example Sentry comment artifact. Companion to [`sentry-integration.md`](./sentry-integration.md). |
